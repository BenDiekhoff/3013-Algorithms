...
void BST::preOrderPrintAux(Node* r, ostream & os) const {
	if (r != nullptr) {
		os << r->val << " ";
		preOrderPrintAux(r->left, os);// print left sub tree
		preOrderPrintAux(r->right, os);//print right sub tree
	}
}

//void BST::flattenAux(Node *& r) {
//	if ( root == nullptr || r->left == nullptr && r->right == nullptr)
//		return;	//base case, second half is importants
//	if (r->left != nullptr) {
//		flattenAux(r->left); //go left
//
//		//we've gone as far left as possible and stepped back up once.
//		Node* tRight = r->right; //temp right pointer to a node
//		//r->right->val = r->left->val;
//		r->right = r->left;
//		r->left = nullptr;
//
//		//search for where to insert nullptr
//		Node* search = r->right;
//		while (search->right != nullptr)
//			search = search->right;
//		search->right = tRight;
//	//	search->right->val = tRight->val;
//	}
//
//	flattenAux(r->right);
//}
...
int BST::iplAux(Node *r, int depth)
{
	if (r == nullptr)
		return 0;

	return (depth + iplAux(r->left, depth + 1) + iplAux(r->right, depth + 1));
}
