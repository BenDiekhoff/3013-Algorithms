...
private:			//CHANGE TO PRIVATE IF BREAKS
	struct Node {
...
public:
	...
	***********************************************************************/
	...
	void preOrderPrint(ostream& os)const {
		if (root == nullptr)  // Tree has no data
			os << "<empty>";
		else
			preOrderPrintAux(root, os);
		os << endl;
	}
...
private:
	...
	void preOrderPrintAux(Node*, ostream&)const;
	void flattenAux(Node*&);
	int iplAux(Node*, int);
};